# Bix

**Cybersecurity Researcher · Bug Bounty Hunter · Offensive Security**

I focus on web application security, vulnerability research, reconnaissance, and penetration testing. I hold active engagements on public bug bounty programs, build my own recon/automation tooling in Python and Bash, and am currently working toward the OSCP.

---

## Areas of Focus

**Offensive Security**  
• Reconnaissance & Enumeration  
• Web Application Security  
• Exploitation & Privilege Escalation  
• Post-Exploitation  
• Active Directory Security (in progress)

**Vulnerability Research**  
• Authentication & Authorization (IDOR)  
• Server-Side Request Forgery (SSRF)  
• Cross-Site Scripting (XSS)  
• SQL Injection  
• File Upload Vulnerabilities  
• API & Business Logic Vulnerabilities

**Security Engineering**  
• Python & Bash automation  
• Linux, networking, Git  
• Recon and testing pipelines

**Cryptography** 

• AES, RSA, hash functions, HMAC, OpenSSL

---

## Tools & Technologies

**Reconnaissance:** subfinder, assetfinder, httpx, katana, waybackurls, dirsearch, WhatWeb  
**Web Security:** ffuf, Burp Suite  
**Subdomain Takeover:** subzy  
**Programming:** Python, Bash

---

## Bug Bounty & Security Research

Actively hunting on public bug bounty programs, currently in the reconnaissance and vulnerability-research phase across multiple engagements. In line with responsible disclosure, I don't publish program names, targets, or findings here until they are triaged/resolved and disclosure is permitted.

---

## Currently Learning

• OSCP (Offensive Security Certified Professional)  
• Advanced Web Application Security  
• Privilege Escalation techniques  
• Active Directory Security  
• Exploit Development

---

## GitHub Stats

![GitHub stats](https://github-readme-stats.vercel.app/api?username=zedx12&show_icons=true&hide_title=true&count_private=true)

---

Open to collaboration on security tooling and write-ups.
